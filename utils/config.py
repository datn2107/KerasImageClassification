...
def _get_section(config_parser: configparser, section: str) -> Dict:
    ...
    for key, value in section_value.items():
        section_value[key] = _get_true_type_from_str(value)
    return section_value

def get_path_from_config(config_path: str) -> Dict:
    path_config = _get_section(_read_config(config_path), 'Path')
    return path_config

def get_data_from_config(config_path: str) -> Dict:
    data_config = _get_section(_read_config(config_path), 'Data')
...
def get_model_from_config(config_path: str) -> Dict:
    ...
    model_config = _get_section(_read_config(config_path), 'Base Model')
...
def get_optimizer_from_config(config_path: str) -> Dict:
    optimizer_config = _get_section(_read_config(config_path), 'Optimizer')
    return optimizer_config

def get_loss_from_config(config_path: str) -> Dict:
    loss_config = _get_section(_read_config(config_path), 'Loss')
    return loss_config

def get_list_metric_from_config(config_path: str) -> List[Dict]:
    list_metric_config = []

    config_parser = _read_config(config_path)
    for section in config_parser.sections():
        if "Metric" not in section:
            continue
        metric_config = _get_section(config_parser, section)
